...
import { Field, TextInput, TextArea, Select, PrimaryButton, SecondaryButton } from "@/components/Field";
import { PaketSummaryCard } from "@/components/PaketSummary";
import { createSesi, deleteSesi, getAllSesi, getPaket, getSesi, updateSesi } from "@/lib/storage";
...
const TIME_OPTIONS = Array.from({ length: 32 }, (_, i) => {
  const totalMinutes = 6 * 60 + i * 30; // 06:00 .. 21:30
...
function DetailSesiInner({ paketId }: { paketId: string }) {
  ...
  const [existing, setExisting] = useState<Sesi | null | undefined>(sesiIdParam ? undefined : null);
  ...
  if (paket === undefined || existing === undefined) return null;
    ...
    return (
      <Screen title="Detail sesi" onBack={goBack}>
        <div className="mt-6 rounded-2xl border border-line bg-white p-5 text-center">
          ...
          <p className="mt-1 text-sm text-ink/60">Silakan pilih tanggal terlebih dahulu.</p>
          <button
  ...
  const locationLabel = paket.mode === "onsite" ? "Tempat (alamat lokasi)" : "Tempat (platform online)";
  const locationPlaceholder =
    paket.mode === "onsite" ? "Contoh: Rumah siswa — Jl. Kaliurang KM 5" : "Contoh: Google Meet";
  ...
  async function handleSave() {
    ...
    try {
      if (existing) {
        await updateSesi(existing.id, { date, startTime, endTime, location: location.trim(), topic: topic.trim() });
      } else {
        await createSesi({ paketId: paket.id, date, startTime, endTime, location: location.trim(), topic: topic.trim() });
      }
  ...
  return (
    <Screen
      title="Detail sesi"
      eyebrow={`${formatFullDate(date)} · sesi ke-${sesiIndex > 0 ? sesiIndex : sortedForPaket.length + 1}`}
      onBack={goBack}
      aside={
        <PaketSummaryCard
          paket={paket}
      ...
      bottom={
        <div className="flex flex-col gap-2">
          ...
          <PrimaryButton
            disabled={!!conflict || !topic.trim() || saving}
            onClick={handleSave}
          >
            {saving
              ? 'Menyimpan…'
