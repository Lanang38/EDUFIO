...
import { Field, TextInput, Select, PillGroup, RadioCard, PrimaryButton } from "@/components/Field";
import { createPaket } from "@/lib/storage";
import { Duration, Mode, PackageSize, Program, PROGRAM_LABEL, MODE_LABEL } from "@/lib/types";
...
export default function DaftarPage() {
  ...
  async function handleSubmit() {
    ...
    setSubmitting(true);
    try {
      const paket = await createPaket({ studentName: studentName.trim(), program, packageSize, duration, mode });
  ...
  return (
    <Screen
      ...
      aside={
        <div className="rounded-2xl border border-line bg-white p-5">
          <p className="text-xs font-semibold uppercase tracking-wide text-teal">Pratinjau paket</p>
    ...
    >
      ...
      <div className="sm:grid sm:grid-cols-2 sm:gap-x-4">
        <Field label="Jumlah sesi dalam paket" required hint="Menentukan kuota sesi yang boleh dijadwalkan.">
          <PillGroup
            options={[4, 8, 12].map((v) => ({ value: v as PackageSize, label: `${v} sesi` }))}
            value={packageSize}
            onChange={setPackageSize}
          />
        </Field>

        <Field label="Durasi per sesi" required hint="Menentukan jam selesai tiap sesi secara otomatis.">
          <PillGroup
            options={[60, 90, 120].map((v) => ({ value: v as Duration, label: `${v} mnt` }))}
            value={duration}
      ...
      <Field label="Mode belajar" required>
        <RadioCard
          options={[
            { value: "onsite" as Mode, label: MODE_LABEL.onsite, description: "Sesi dilakukan di lokasi siswa" },
            { value: "online" as Mode, label: MODE_LABEL.online, description: "Sesi dilakukan lewat panggilan video" },
          ]}
          value={mode}
          onChange={setMode}
        />
      ...
      <p className="mt-2 text-xs text-ink/40">
        Jumlah sesi &amp; durasi mengunci kuota dan jam selesai tiap sesi. Data ini tidak bisa diubah
        setelah sesi mulai dijadwalkan.
      </p>
    </Screen>
  );
}
